...
from db.queries import get_clinic_id, book_new_appointment, lookup_active_appointment
...
async def book_appointment(params: FunctionCallParams, doctor_id: str, patient_name: str, start_time_iso: str, phone: str, force_book: bool = False):
    ...
    lock_key = f"booking_lock:doctor_{doctor_id}:time_{start_time_iso}"
    ...
    try:
        ...
        # --- NEW WARNING LOGIC WITH DOCTOR DETAILS ---
        if str(appt_id).startswith("HAS_OTHER_APPOINTMENT"):
            # Unpack the 6 values returned from the database
            parts = appt_id.split("|")
        ...
        if payment_link:
            whatsapp_msg = (
                f"🏥 *Mithra Hospitals*\n\n"
                f"Hi {patient_name}, your appointment is tentatively booked for "
